package com.rahmitufanoglu.mygames.xbox
...
import com.rahmitufanoglu.mygames.model.Xbox
...
class XboxActivity : AppCompatActivity(), SwipeRefreshLayout.OnRefreshListener {

    private var gamesList: ArrayList<Xbox>? = null
    private var sortOption: Int = 1 // Default sorting by release date
    private lateinit var xboxAdapter: XboxAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        swipe_refresh_layout_platform.run {
            setOnRefreshListener(this@XboxActivity)
        ...
        searchView.run {
            ...
            setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                ...
                override fun onQueryTextChange(newText: String): Boolean {
                    if (newText.isEmpty()) {
                        xboxAdapter.setFilter(gamesList!!)
                    } else {
                        val tmp = ArrayList<Xbox>()
        ...
        when (item.itemId) {
            ...
            R.id.menu_release_date_sort -> {
                if (gamesList != null) {
                    ...
                    xboxAdapter.notifyDataSetChanged()
                    item.isChecked = true
                    sortOption = 1
                } else {
        ...
        call.enqueue(object : Callback<GameDataRepo> {
            override fun onResponse(call: Call<GameDataRepo>, response: Response<GameDataRepo>) {
                if (response.isSuccessful) {
                    gamesList = response.body()!!.xbox as ArrayList<Xbox>
                    xboxAdapter = XboxAdapter(this@XboxActivity, gamesList!!)
                    when (sortOption) {
                        1 -> Collections.sort(gamesList, compareRelease())
                        2 -> Collections.sort(gamesList, compareTitle())
                        3 -> gamesList?.shuffle()
                    }
                    recycler_view.adapter = xboxAdapter
                } else {
