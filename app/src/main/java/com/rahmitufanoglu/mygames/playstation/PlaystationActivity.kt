package com.rahmitufanoglu.mygames.playstation
...
import com.rahmitufanoglu.mygames.model.Playstation
...
class PlaystationActivity : AppCompatActivity(), SwipeRefreshLayout.OnRefreshListener {

    private var gamesList: ArrayList<Playstation>? = null
    private var sortOption: Int = 1 // Default sorting by release date
    private lateinit var playstationAdapter: PlaystationAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        swipe_refresh_layout_platform.run {
            setOnRefreshListener(this@PlaystationActivity)
    ...
    private fun setActionBar() {
        ...
        supportActionBar?.run {
            ...
            title = getString(R.string.playstation)
        }
    }
        ...
        searchView.run {
            ...
            setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                ...
                override fun onQueryTextChange(newText: String): Boolean {
                    if (newText.isEmpty()) {
                        playstationAdapter.setFilter(gamesList!!)
                    } else {
                        val tmp = ArrayList<Playstation>()
                        ...
                        playstationAdapter.setFilter(tmp)
                    }
                    return false
                }
            })
        ...
        call.enqueue(object : Callback<GameDataRepo> {
            override fun onResponse(call: Call<GameDataRepo>, response: Response<GameDataRepo>) {
                if (response.isSuccessful) {
                    gamesList = response.body()!!.playstation as ArrayList<Playstation>
                    playstationAdapter = PlaystationAdapter(this@PlaystationActivity, gamesList!!)
                    when (sortOption) {
                        1 -> Collections.sort(gamesList, compareRelease())
                        2 -> Collections.sort(gamesList, compareTitle())
                        3 -> gamesList?.shuffle()
                    }
                    recycler_view.adapter = playstationAdapter
                } else {
