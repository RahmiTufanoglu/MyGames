package com.rahmitufanoglu.mygames.pc
...
import com.rahmitufanoglu.mygames.model.Pc
...
class PcActivity : AppCompatActivity(), SwipeRefreshLayout.OnRefreshListener {

    private var gamesList: ArrayList<Pc>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        swipe_refresh_layout_platform.run {
            setOnRefreshListener(this@PcActivity)
        ...
        searchView.run {
            ...
            setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                ...
                override fun onQueryTextChange(newText: String): Boolean {
                    if (newText.isEmpty()) {
                        pcAdapter.setFilter(gamesList!!)
                    } else {
                        val tmp = ArrayList<Pc>()
        ...
        call.enqueue(object : Callback<GameDataRepo> {
            override fun onResponse(call: Call<GameDataRepo>, response: Response<GameDataRepo>) {
                if (response.isSuccessful) {
                    gamesList = response.body()!!.pc as ArrayList<Pc>
                    pcAdapter = PcAdapter(this@PcActivity, gamesList!!)
                    when (sortOption) {
                        1 -> Collections.sort(gamesList, compareRelease())
                        2 -> Collections.sort(gamesList, compareTitle())
                        3 -> gamesList?.shuffle()
                    }
                    recycler_view.adapter = pcAdapter
                } else {
